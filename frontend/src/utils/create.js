...
export const CreateData = async (bucketName, address, signer,chain, switchNetworkAsync,data, type) => {
    ...
    // })
        ...
        data = {title: data.title, shortText: data.shortText, bannerUrl: data.bannerUrl, text: data.text, audio: data.audio}
    ...
    const userTx = await client.object.createObject(
      ...
      // upload audio
      const buffAudio = Buffer.from(data.audio.toString("utf8")).toString();
      const audioResponse  = await api.post("/checksums",buffAudio)
